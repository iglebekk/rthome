...
<x-app.page-header :title="$event->name" :$description :eyebrow="$club->name">
    <x-slot:actions>
        <x-app.link-button :href="$backUrl" variant="ghost">{{ __('app.actions.back') }}</x-app.link-button>
        ...
        @if ($editUrl)
            <x-app.link-button :href="$editUrl" icon="pencil-square">{{ __('events.actions.edit') }}</x-app.link-button>
        @endif
    </x-slot:actions>
</x-app.page-header>
    ...
    <x-app.card class="overflow-hidden">
        ...
        @else
            <div class="grid aspect-[16/9] place-items-center bg-gradient-to-br from-emerald-100 to-stone-100 dark:from-emerald-950 dark:to-zinc-900">
                <x-app.icon name="calendar-days" class="size-12 text-emerald-700 dark:text-emerald-300" />
            </div>
        @endif
